...
export default function SkillsSection() {
  return (
    <section id="skills" className="scroll-mt-16 bg-background py-20 text-foreground md:py-28">
          ...
          {skills.map(({ label, items }, index) => (
            <MotionWrapper key={label} delay={index * 0.05}>
              <div className="grid gap-2 py-5 md:grid-cols-[180px_1fr] md:gap-8 md:py-6">
                ...
                <ul className="flex flex-wrap gap-x-3 gap-y-2 text-xl md:text-2xl font-display leading-tight">
                  {items.map((skill) => (
                    <li
                      key={skill}
